...
// Return boolean telling if a file was created within the given period of hours
lib.recentlyCreatedFile = function(dir, file, hours, callback) {
    let isRecent = false;
    fs.stat(lib.baseDir+dir+'/'+file+'.json', function(err, statsData) {
        if (!err && statsData) {
            const creationTime = new Date(util.inspect(statsData.birthtime));
            const minutesDiff = new Date(Date.now() - creationTime.getTime()).getMinutes();
            isRecent =  minutesDiff < (hours * 60);
            callback(false, isRecent);
        } else {
